...
// Content gap analyzer surfacing opportunities from competitor data
// Feature: content-gap-analyzer
...
router.post('/analyze', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({ success: true, feature: 'content-gap-analyzer', ...result });
  } catch (err: any) {
...
router.post('/batch', async (req: Request, res: Response) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Content gap analyzer surfacing opportunities from competitor data. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', (req: Request, res: Response) => {
  res.json({ feature: 'content-gap-analyzer', title: 'Content gap analyzer surfacing opportunities from competitor data', project: 'AiVideoScriptContent' });
});
