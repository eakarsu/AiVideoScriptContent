...
// Direct YouTube/TikTok/Instagram API publishing with scheduling
// Feature: platform-publishing
...
router.post('/analyze', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({ success: true, feature: 'platform-publishing', ...result });
  } catch (err: any) {
...
router.post('/batch', async (req: Request, res: Response) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Direct YouTube/TikTok/Instagram API publishing with scheduling. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', (req: Request, res: Response) => {
  res.json({ feature: 'platform-publishing', title: 'Direct YouTube/TikTok/Instagram API publishing with scheduling', project: 'AiVideoScriptContent' });
});
