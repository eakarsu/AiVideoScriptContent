...
// Multi-platform optimizer generating platform-specific versions
// Feature: multi-platform-optimizer
...
router.post('/analyze', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({ success: true, feature: 'multi-platform-optimizer', ...result });
  } catch (err: any) {
...
router.post('/batch', async (req: Request, res: Response) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Multi-platform optimizer generating platform-specific versions. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', (req: Request, res: Response) => {
  res.json({ feature: 'multi-platform-optimizer', title: 'Multi-platform optimizer generating platform-specific versions', project: 'AiVideoScriptContent' });
});
