...
import { generateVideoSummary } from '../services/openrouter.service';
...
export const getAll = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const { count, rows } = await VideoSummary.findAndCountAll({
      where,
      order,
      limit: pageSize,
...
export const getById = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const item = await VideoSummary.findOne({
      where: { id: req.params.id, userId: req.userId },
    });
    if (!item) {
      res.status(404).json({ error: 'Video summary not found' });
      return;
    }
    res.json(item);
  } catch (error) {
...
export const create = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { videoTitle, videoUrl, videoTranscript, summaryLength, includeKeyPoints, aiOutput, status, scheduledAt } = req.body;
    const item = await VideoSummary.create({
      userId: req.userId!,
      videoTitle,
      videoUrl,
      videoTranscript,
      summaryLength,
      includeKeyPoints: includeKeyPoints ?? true,
      aiOutput,
      status: status || 'draft',
...
export const generate = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { videoTitle, videoTranscript, summaryLength, includeKeyPoints } = req.body;
    const aiOutput = await generateVideoSummary(videoTitle, videoTranscript, summaryLength, includeKeyPoints);
    res.json({ aiOutput });
  } catch (error) {
...
export const bulkDelete = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const deleted = await VideoSummary.destroy({
...
export const bulkStatus = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const [updated] = await VideoSummary.update(updateData, {
      where: { id: { [Op.in]: ids }, userId: req.userId },
    });
