...
export default function GapNoMultimodalScriptToStoryboardGenerator() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-multimodal-script-to-storyboard-generator/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-multimodal-script-to-storyboard-generator', project: 'AiVideoScriptContent' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-multimodal-script-to-storyboard-generator/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No multimodal script-to-storyboard generator</h1>
