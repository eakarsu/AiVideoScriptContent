...
export default function PodcastTranscriptsPage() {
  const [items, setItems] = useState<PodcastTranscript[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedItem, setSelectedItem] = useState<PodcastTranscript | null>(null);
  const [showForm, setShowForm] = useState(false);
  const [editingItem, setEditingItem] = useState<PodcastTranscript | null>(null);
  ...
  const feature = getFeatureById('podcast-transcripts')!;
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const response = await api.get(`/podcast-transcripts?${params.toString()}`);
      const data = response.data;
      setItems(data.data || data);
      setTotalPages(data.pagination?.totalPages || 1);
    } catch (error) {
      addToast('Failed to load transcripts', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (data: any) => {
    try {
      if (editingItem) {
        await api.put(`/podcast-transcripts/${editingItem.id}`, data);
        addToast('Transcript updated successfully', 'success');
      } else {
        await api.post('/podcast-transcripts', data);
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!ok) return;
    try {
      await api.delete(`/podcast-transcripts/${id}`);
      addToast('Transcript deleted', 'success');
      await fetchItems();
      setSelectedItem(null);
    } catch (error) {
      addToast('Failed to delete transcript', 'error');
    }
  };
  ...
  const handleStatusSave = async (status: 'draft' | 'scheduled' | 'published', scheduledAt: string | null) => {
    ...
    try {
      await api.put(`/podcast-transcripts/${selectedItem.id}`, { status, scheduledAt });
  ...
  const handleEdit = (item: PodcastTranscript) => {
    setEditingItem(item);
    setSelectedItem(null);
    setShowForm(true);
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (!ok) return;
    try {
      await api.post('/podcast-transcripts/bulk-delete', { ids: selectedIds });
      addToast(`${selectedIds.length} transcripts deleted`, 'success');
      setSelectedIds([]);
      await fetchItems();
    } catch (error) {
      addToast('Failed to delete selected transcripts', 'error');
    }
  };

  const handleBulkStatus = async (status: 'draft' | 'scheduled' | 'published') => {
    try {
      await api.post('/podcast-transcripts/bulk-status', { ids: selectedIds, status });
      addToast(`${selectedIds.length} transcripts updated to ${status}`, 'success');
      setSelectedIds([]);
      await fetchItems();
    } catch (error) {
      addToast('Failed to update status', 'error');
  ...
  const filteredItems = items.filter((item) => {
    ...
    return item.podcastTitle.toLowerCase().includes(search);
  });
  ...
  return (
    <PageLayout
      ...
      subtitle="Transcribe and format podcast episodes"
      icon=""
      actions={
        <div className="flex items-center gap-2">
          <button onClick={() => exportToCSV(filteredItems, 'podcast-transcripts', exportColumns)} className="btn-secondary text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
            </svg>
            CSV
          </button>
          <button onClick={() => exportToPDF(filteredItems, 'podcast-transcripts', exportColumns, 'Podcast Transcripts')} className="btn-secondary text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
            </svg>
            PDF
    ...
    >
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center" style={{ boxShadow: '0 1px 3px 0 rgb(0 0 0 / 0.04)' }}>
          ...
          <p className="text-gray-500 mb-1 font-medium">No transcripts found</p>
          <p className="text-sm text-gray-400 mb-5">Create your first transcript to get started</p>
      ...
      {selectedItem && !showStatusModal && (
        <Modal title="Podcast Transcript" onClose={() => setSelectedItem(null)}>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold">{selectedItem.podcastTitle}</h3>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium">{selectedItem.outputFormat}</p>
              </div>
              <div>
                <span className="text-gray-500">Timestamps:</span>
                <p className="font-medium">{selectedItem.includeTimestamps ? 'Included' : 'Not included'}</p>
              </div>
              <div>
                <span className="text-gray-500">Speaker Labels:</span>
                <p className="font-medium">{selectedItem.includeSpeakerLabels ? 'Included' : 'Not included'}</p>
              </div>
              {selectedItem.podcastUrl && (
                <div>
                  <span className="text-gray-500">Podcast URL:</span>
                  <p className="font-medium text-primary-600 truncate">{selectedItem.podcastUrl}</p>
                </div>
              )}
            </div>

            {selectedItem.aiOutput && (
              <AIOutputDisplay content={selectedItem.aiOutput} title="Podcast Transcript" />
            )}
      ...
      {/* Form Modal */}
      {showForm && (
        <Modal title={editingItem ? 'Edit Transcript' : 'Create Podcast Transcript'} onClose={() => { setShowForm(false); setEditingItem(null); }}>
          <ItemForm
            feature={feature}
            onSave={handleSave}
