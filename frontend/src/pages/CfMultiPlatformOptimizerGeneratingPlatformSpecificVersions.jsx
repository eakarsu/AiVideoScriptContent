...
// Feature: Multi-platform optimizer generating platform-specific versions
...
export default function CfMultiPlatformOptimizerGeneratingPlatformSpecificVersions() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-platform-optimizer-generating-platform-specific-versions/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-multi-platform-optimizer-generating-platform-specific-versions', project: 'AiVideoScriptContent' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-platform-optimizer-generating-platform-specific-versions/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Multi-platform optimizer generating platform-specific versions</h1>
