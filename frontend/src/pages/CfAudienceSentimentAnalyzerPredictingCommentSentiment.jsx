...
export default function CfAudienceSentimentAnalyzerPredictingCommentSentiment() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-audience-sentiment-analyzer-predicting-comment-sentiment/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-audience-sentiment-analyzer-predicting-comment-sentiment', project: 'AiVideoScriptContent' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-audience-sentiment-analyzer-predicting-comment-sentiment/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Audience sentiment analyzer predicting comment sentiment</h1>
