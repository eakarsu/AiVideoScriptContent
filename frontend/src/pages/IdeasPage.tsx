...
export default function IdeasPage() {
  ...
  const feature = getFeatureById('ideas')!;
  ...
  const handleSave = async (data: any) => {
    try {
      if (editingItem) {
        await api.put(`/ideas/${editingItem.id}`, data);
  ...
  const handleStatusSave = async (status: 'draft' | 'scheduled' | 'published', scheduledAt: string | null) => {
    ...
    try {
      await api.put(`/ideas/${selectedItem.id}`, { status, scheduledAt });
  ...
  const handleBulkDelete = async () => {
    ...
    if (!ok) return;
    try {
      await api.post('/ideas/bulk-delete', { ids: selectedIds });
      addToast(`${selectedIds.length} ideas deleted`, 'success');
      setSelectedIds([]);
      await fetchItems();
    } catch (error) {
      addToast('Failed to delete selected ideas', 'error');
    }
  };

  const handleBulkStatus = async (status: 'draft' | 'scheduled' | 'published') => {
    try {
      await api.post('/ideas/bulk-status', { ids: selectedIds, status });
      addToast(`${selectedIds.length} ideas updated to ${status}`, 'success');
      setSelectedIds([]);
      await fetchItems();
    } catch (error) {
      addToast('Failed to update status', 'error');
  ...
  const filteredItems = items.filter((item) => {
    ...
    return (
      ...
      item.targetAudience?.toLowerCase().includes(search)
    );
  });
  ...
  return (
    <PageLayout
      title="Video Ideas"
      subtitle="Brainstorm content ideas"
      icon=""
      actions={
        <div className="flex items-center gap-2">
          <button onClick={() => exportToCSV(filteredItems, 'ideas', exportColumns)} className="btn-secondary text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
            </svg>
            CSV
          </button>
          <button onClick={() => exportToPDF(filteredItems, 'ideas', exportColumns, 'Video Ideas')} className="btn-secondary text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
            </svg>
            PDF
    ...
    >
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center" style={{ boxShadow: '0 1px 3px 0 rgb(0 0 0 / 0.04)' }}>
          ...
          <p className="text-gray-500 mb-1 font-medium">No ideas found</p>
          <p className="text-sm text-gray-400 mb-5">Brainstorm your first video idea</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredItems.map((item) => (
              <ContentCard
                ...
                id={item.id}
                title={item.niche}
                subtitle={`${item.platform} - ${item.contentType}`}
      ...
      {selectedItem && !showStatusModal && (
        <Modal title="Idea Details" onClose={() => setSelectedItem(null)}>
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">{selectedItem.niche}</h3>
              <StatusBadge status={selectedItem.status || 'draft'} />
            </div>

            <div className="grid grid-cols-2 gap-4">
              {[
                { label: 'Niche', value: selectedItem.niche },
                { label: 'Platform', value: selectedItem.platform },
                { label: 'Content Type', value: selectedItem.contentType },
                { label: 'Target Audience', value: selectedItem.targetAudience },
              ].map((field) => (
            ...
            {selectedItem.aiOutput && (
              <AIOutputDisplay content={selectedItem.aiOutput} title="Generated Ideas" />
            )}
      ...
      {/* Form Modal */}
      {showForm && (
        <Modal title={editingItem ? 'Edit Idea' : 'New Idea'} onClose={() => { setShowForm(false); setEditingItem(null); }}>
          <ItemForm
            feature={feature}
            onSave={handleSave}
