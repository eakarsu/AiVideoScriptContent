...
export default function CtasPage() {
  ...
  const [selectedItem, setSelectedItem] = useState<Cta | null>(null);
  const [showForm, setShowForm] = useState(false);
  const [editingItem, setEditingItem] = useState<Cta | null>(null);
  ...
  const feature = getFeatureById('ctas')!;
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const response = await api.get(`/ctas?${params.toString()}`);
      const data = response.data;
      setItems(data.data || data);
      setTotalPages(data.pagination?.totalPages || 1);
    } catch (error) {
      addToast('Failed to load CTAs', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (data: any) => {
    try {
      if (editingItem) {
        await api.put(`/ctas/${editingItem.id}`, data);
        addToast('CTA updated successfully', 'success');
      } else {
        await api.post('/ctas', data);
        addToast('CTA created successfully', 'success');
      }
      await fetchItems();
      setShowForm(false);
      setEditingItem(null);
    } catch (error) {
      addToast('Failed to save CTA', 'error');
      throw error;
    }
  };
  ...
  const handleStatusSave = async (status: 'draft' | 'scheduled' | 'published', scheduledAt: string | null) => {
    ...
    try {
      await api.put(`/ctas/${selectedItem.id}`, { status, scheduledAt });
  ...
  const handleBulkDelete = async () => {
    ...
    if (!ok) return;
    try {
      await api.post('/ctas/bulk-delete', { ids: selectedIds });
      addToast(`${selectedIds.length} CTAs deleted`, 'success');
      setSelectedIds([]);
      await fetchItems();
    } catch (error) {
      addToast('Failed to delete selected CTAs', 'error');
    }
  };

  const handleBulkStatus = async (status: 'draft' | 'scheduled' | 'published') => {
    try {
      await api.post('/ctas/bulk-status', { ids: selectedIds, status });
      addToast(`${selectedIds.length} CTAs updated to ${status}`, 'success');
      setSelectedIds([]);
      await fetchItems();
    } catch (error) {
      addToast('Failed to update status', 'error');
  ...
  return (
    <PageLayout
      title="CTA Optimizer"
      subtitle="Create compelling call-to-actions"
      icon=""
      actions={
        <div className="flex items-center gap-2">
          <button onClick={() => exportToCSV(filteredItems, 'ctas', exportColumns)} className="btn-secondary text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
            </svg>
            CSV
          </button>
          <button onClick={() => exportToPDF(filteredItems, 'ctas', exportColumns, 'CTAs')} className="btn-secondary text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
            </svg>
            PDF
    ...
    >
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center" style={{ boxShadow: '0 1px 3px 0 rgb(0 0 0 / 0.04)' }}>
          ...
          <p className="text-gray-500 mb-1 font-medium">No CTAs found</p>
          <p className="text-sm text-gray-400 mb-5">Create your first CTA to get started</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredItems.map((item) => (
              <ContentCard
                ...
                subtitle={`${item.platform} - ${item.goal}`}
      ...
      {selectedItem && !showStatusModal && (
        <Modal title="CTA Details" onClose={() => setSelectedItem(null)}>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <span className="text-gray-500">Target Action:</span>
                <p className="font-medium">{selectedItem.targetAction}</p>
              </div>
            </div>

            {selectedItem.aiOutput && (
              <AIOutputDisplay content={selectedItem.aiOutput} title="Generated CTAs" />
            )}
      ...
      {/* Form Modal */}
      {showForm && (
        <Modal title={editingItem ? 'Edit CTA' : 'New CTA'} onClose={() => { setShowForm(false); setEditingItem(null); }}>
          <ItemForm
            feature={feature}
            onSave={handleSave}
