...
export default function GapTsvReports0AiButRoutesSuggest() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-tsv-reports-0-ai-but-routes-suggest-under-reported/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-tsv-reports-0-ai-but-routes-suggest-under-reported', project: 'AiVideoScriptContent' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-tsv-reports-0-ai-but-routes-suggest-under-reported/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">TSV reports 0 AI but routes suggest under-reported; likely missing AI for hooks generation, viral prediction, trend forecasting, SEO suggest</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AiVideoScriptContent</p>
      </div>
