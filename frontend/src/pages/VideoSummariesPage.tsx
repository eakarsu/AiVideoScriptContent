...
export default function VideoSummariesPage() {
  ...
  const [selectedItem, setSelectedItem] = useState<VideoSummary | null>(null);
  const [showForm, setShowForm] = useState(false);
  const [editingItem, setEditingItem] = useState<VideoSummary | null>(null);
  ...
  const feature = getFeatureById('video-summaries')!;
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const response = await api.get(`/video-summaries?${params.toString()}`);
      const data = response.data;
      setItems(data.data || data);
      setTotalPages(data.pagination?.totalPages || 1);
    } catch (error) {
      addToast('Failed to load summaries', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (data: any) => {
    try {
      if (editingItem) {
        await api.put(`/video-summaries/${editingItem.id}`, data);
        addToast('Summary updated successfully', 'success');
      } else {
        await api.post('/video-summaries', data);
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!ok) return;
    try {
      await api.delete(`/video-summaries/${id}`);
  ...
  const handleStatusSave = async (status: 'draft' | 'scheduled' | 'published', scheduledAt: string | null) => {
    ...
    try {
      await api.put(`/video-summaries/${selectedItem.id}`, { status, scheduledAt });
  ...
  const handleEdit = (item: VideoSummary) => {
    setEditingItem(item);
    setSelectedItem(null);
    setShowForm(true);
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (!ok) return;
    try {
      await api.post('/video-summaries/bulk-delete', { ids: selectedIds });
      addToast(`${selectedIds.length} summaries deleted`, 'success');
      setSelectedIds([]);
      await fetchItems();
    } catch (error) {
      addToast('Failed to delete selected summaries', 'error');
    }
  };

  const handleBulkStatus = async (status: 'draft' | 'scheduled' | 'published') => {
    try {
      await api.post('/video-summaries/bulk-status', { ids: selectedIds, status });
      addToast(`${selectedIds.length} summaries updated to ${status}`, 'success');
      setSelectedIds([]);
      await fetchItems();
    } catch (error) {
      addToast('Failed to update status', 'error');
  ...
  const filteredItems = items.filter((item) => {
    ...
    return item.videoTitle.toLowerCase().includes(search);
  });
  ...
  return (
    <PageLayout
      ...
      subtitle="Generate summaries from video content"
      icon=""
      actions={
        <div className="flex items-center gap-2">
          <button onClick={() => exportToCSV(filteredItems, 'video-summaries', exportColumns)} className="btn-secondary text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
            </svg>
            CSV
          </button>
          <button onClick={() => exportToPDF(filteredItems, 'video-summaries', exportColumns, 'Video Summaries')} className="btn-secondary text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
            </svg>
            PDF
    ...
    >
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center" style={{ boxShadow: '0 1px 3px 0 rgb(0 0 0 / 0.04)' }}>
          ...
          <p className="text-gray-500 mb-1 font-medium">No summaries found</p>
          <p className="text-sm text-gray-400 mb-5">Create your first summary to get started</p>
      ...
      {selectedItem && !showStatusModal && (
        <Modal title="Video Summary" onClose={() => setSelectedItem(null)}>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium">{selectedItem.summaryLength}</p>
              </div>
              <div>
                <span className="text-gray-500">Key Points:</span>
                <p className="font-medium">{selectedItem.includeKeyPoints ? 'Included' : 'Not included'}</p>
              </div>
              {selectedItem.videoUrl && (
                <div className="col-span-2">
                  <span className="text-gray-500">Video URL:</span>
                  <p className="font-medium text-primary-600 truncate">{selectedItem.videoUrl}</p>
                </div>
              )}
            </div>

            {selectedItem.aiOutput && (
              <AIOutputDisplay content={selectedItem.aiOutput} title="Video Summary" />
            )}
      ...
      {/* Form Modal */}
      {showForm && (
        <Modal title={editingItem ? 'Edit Summary' : 'Create Video Summary'} onClose={() => { setShowForm(false); setEditingItem(null); }}>
          <ItemForm
            feature={feature}
            onSave={handleSave}
