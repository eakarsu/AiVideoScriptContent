...
// Feature: Content gap analyzer surfacing opportunities from competitor data
...
export default function CfContentGapAnalyzerSurfacingOpportunitiesFromCompetitor() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-content-gap-analyzer-surfacing-opportunities-from-competitor-data/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-content-gap-analyzer-surfacing-opportunities-from-competitor-data', project: 'AiVideoScriptContent' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-content-gap-analyzer-surfacing-opportunities-from-competitor-data/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Content gap analyzer surfacing opportunities from competitor data</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AiVideoScriptContent</p>
      </div>
