...
// Feature: Direct YouTube/TikTok/Instagram API publishing with scheduling
...
export default function CfDirectYoutubeTiktokInstagramApiPublishingWith() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-direct-youtube-tiktok-instagram-api-publishing-with-scheduling/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-direct-youtube-tiktok-instagram-api-publishing-with-scheduling', project: 'AiVideoScriptContent' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-direct-youtube-tiktok-instagram-api-publishing-with-scheduling/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Direct YouTube/TikTok/Instagram API publishing with scheduling</h1>
