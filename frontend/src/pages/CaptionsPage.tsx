...
export default function CaptionsPage() {
  ...
  const [selectedItem, setSelectedItem] = useState<Caption | null>(null);
  ...
  const feature = getFeatureById('captions')!;
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const response = await api.get(`/captions?${params.toString()}`);
      const data = response.data;
      setItems(data.data || data);
      setTotalPages(data.pagination?.totalPages || 1);
    } catch (error) {
      addToast('Failed to load captions', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (data: any) => {
    try {
      if (editingItem) {
        await api.put(`/captions/${editingItem.id}`, data);
        addToast('Caption updated successfully', 'success');
      } else {
        await api.post('/captions', data);
        addToast('Caption created successfully', 'success');
      }
      await fetchItems();
      setShowForm(false);
      setEditingItem(null);
  ...
  const handleDelete = async (id: number) => {
    const ok = await confirm({
      ...
      message: 'Are you sure you want to delete this caption? This action cannot be undone.',
      confirmLabel: 'Delete',
      variant: 'danger',
    });
    if (!ok) return;
    try {
      ...
      addToast('Caption deleted', 'success');
      await fetchItems();
      setSelectedItem(null);
    } catch (error) {
      addToast('Failed to delete caption', 'error');
    }
  };
  ...
  const handleStatusSave = async (status: 'draft' | 'scheduled' | 'published', scheduledAt: string | null) => {
    ...
    try {
      await api.put(`/captions/${selectedItem.id}`, { status, scheduledAt });
  ...
  const handleBulkDelete = async () => {
    ...
    if (!ok) return;
    try {
      await api.post('/captions/bulk-delete', { ids: selectedIds });
      addToast(`${selectedIds.length} captions deleted`, 'success');
      setSelectedIds([]);
      await fetchItems();
    } catch (error) {
      addToast('Failed to delete selected captions', 'error');
    }
  };

  const handleBulkStatus = async (status: 'draft' | 'scheduled' | 'published') => {
    try {
      await api.post('/captions/bulk-status', { ids: selectedIds, status });
      addToast(`${selectedIds.length} captions updated to ${status}`, 'success');
      setSelectedIds([]);
      await fetchItems();
    } catch (error) {
      addToast('Failed to update status', 'error');
  ...
  return (
    <PageLayout
      title="Captions"
      subtitle="Generate engaging captions for your videos"
      icon=""
      actions={
        <div className="flex items-center gap-2">
          <button onClick={() => exportToCSV(filteredItems, 'captions', exportColumns)} className="btn-secondary text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
            </svg>
            CSV
          </button>
          <button onClick={() => exportToPDF(filteredItems, 'captions', exportColumns, 'Captions')} className="btn-secondary text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
            </svg>
            PDF
    ...
    >
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center" style={{ boxShadow: '0 1px 3px 0 rgb(0 0 0 / 0.04)' }}>
          ...
          <p className="text-gray-500 mb-1 font-medium">No captions found</p>
          <p className="text-sm text-gray-400 mb-5">Create your first caption to get started</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredItems.map((item) => (
              <ContentCard
                ...
                subtitle={`${item.platform} - ${item.captionStyle}`}
      ...
      {selectedItem && !showStatusModal && (
        <Modal title="Caption Details" onClose={() => setSelectedItem(null)}>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">{selectedItem.captionStyle}</p>
              </div>
              <div>
                <span className="text-gray-500">Emojis:</span>
                <p className="font-medium">{selectedItem.includeEmojis ? 'Yes' : 'No'}</p>
              </div>
            </div>

            {selectedItem.aiOutput && (
              <AIOutputDisplay content={selectedItem.aiOutput} title="Generated Caption" />
            )}
      ...
      {/* Form Modal */}
      {showForm && (
        <Modal title={editingItem ? 'Edit Caption' : 'New Caption'} onClose={() => { setShowForm(false); setEditingItem(null); }}>
          <ItemForm
            feature={feature}
            onSave={handleSave}
